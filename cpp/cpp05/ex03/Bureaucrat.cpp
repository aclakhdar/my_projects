...
void	Bureaucrat::signAForm(AForm& a)
{
	if(a.is_sign())
	{
		std::cout<< "the AForm is already signed \n";
		return ;
	}
	...
	{
		std::cerr << "the AForm : " << "\"" + a.get_name() + "\" " << " signed failed : " << e.what() << '\n';
	}
...
void Bureaucrat::executeForm(AForm const &form) const
{
	...
	{
		form.execute(*this);
		std::cout << "the AForm : " << "\"" + form.get_name() + "\" " << " executed successfully" << '\n';
	}
	catch (AForm::GradeTooLowException &e)
	{
		std::cerr << "the AForm : " << "\"" + form.get_name() + "\" " << " executed failed : " << e.what() << '\n';
	}
